$(function() {
	...
	var editIndex = -1;
	var scndCate = 21;
	var table = $('#professionaldg');

	// 初始化搜索框
	$('#professionalsb').searchbox({
		prompt : '输入课程名称',
	...
	// 结束编辑事件（保存）
	$('#profssave').click(function() {
		if (editIndex >= 0) {
			table.datagrid('endEdit', editIndex);
		}
		operateDatas(table, '/TrainingProgramManager/updatePrograms.do');
		$('#profssave,#profsredo').hide();
	})
	// 结束编辑事件（撤销）
	$('#profsredo').click(function() {
		$('#profssave,#profsredo').hide();
		table.datagrid('load');
	})
	// 显示新增窗口
	$('#profsadd').click(function() {
		showProDialog('#professionaldg', scndCate);
	});
	// 删除选中
	$('#profsdel').click(function() {
